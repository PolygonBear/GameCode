...
ALadder::ALadder()
{
	...
	StepMeshComponent = CreateDefaultSubobject<UInstancedStaticMeshComponent>(TEXT("Steps"));
...
void ALadder::OnConstruction(const FTransform& Transform)
{
	...
	UStaticMesh* StepMesh = StepMeshComponent->GetStaticMesh();
	if(IsValid(StepMesh))
	{
		float MeshWidth = StepMesh->GetBoundingBox().GetSize().Y;
		if(!FMath::IsNearlyZero(MeshWidth))
		{
			StepMeshComponent->SetRelativeScale3D(FVector(1.0f, LadderWidth / MeshWidth,1.0f));
		}
	}
